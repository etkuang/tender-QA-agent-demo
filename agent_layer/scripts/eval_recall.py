    ...
    async def evaluate_strategy(self, strategy: str, eval_set_path: Path, top_k: int = 10) -> StrategyResult:
        ...
        try:
            ...
            for category in categories:
                ...
                total_count = len(cat_results)  # 该类别总问题数（包括失败的）
                valid_count = len([r for r in cat_results if r.retrieved_docs])  # 实际成功的
...
class FusionComparator:
    ...
    def save_comparison_report(self):
        """保存对比报告"""
        output_dir = Path("agent_layer/data/eval_results")
        ...
        comparison = {
            ...
            "note": "失败查询计为0分，确保公平对比。评估模式下 min_final_score=0.0",
            "results": {}
        }
