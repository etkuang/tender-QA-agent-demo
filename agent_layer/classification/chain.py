...
from agent_layer.schemas import QuestionDecomposition
...
class QuestionClassifier:
    ...
    async def classify(self, question: str, context_summary: str = "") -> QuestionDecomposition:
        try:
            result = await self.chain.with_retry(
            ...
            ).ainvoke(
                {
                    ...
                    "schema": json.dumps(QuestionDecomposition.model_json_schema(), ensure_ascii=False),
                }
            )
        except Exception as exc:
            logger.exception("structured decomposition failed")
            raise_model_error(exc, ClassificationError)
        if not isinstance(result, QuestionDecomposition):
            logger.error("decomposition returned unexpected type | type=%s", type(result).__name__)
