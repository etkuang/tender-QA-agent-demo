from agent_layer.domain.retrieval.fusion_policy import HybridFusionV2
...
from agent_layer.domain.retrieval.chinese_number import chinese_number_converter  # ← 新增
import re
...
class HybridRetriever:
    ...
    def search_article_exact(self, law_name: str, article_num: str) -> List[Dict]:
        """Exact article lookup compatible with both metadata schemas."""
        query = f"{law_name} 第{article_num}条" if law_name else f"第{article_num}条"

        filters = [{"article_num": article_num, "type": "law_article"}]
        if article_num.isdigit():
            numeric_article = int(article_num)
            filters.append({"article_id": numeric_article, "chunk_type": "parent"})
            filters.append({"article_id": numeric_article})
        ...
        # Keep text fallback for cases where metadata is missing.
        for r in results:
            text = r.get("text", "")
